 ...
 * a task through beginTask, setting the Total number once, and then updating the Progress number every few seconds
 * (depending on the granularity you want/need).  When you have completed the task, call complete() and the task may then
...
public abstract class Task {
	...
	/**
	 * Create a new Task with the current TaskFactory.  The task is ready to be interacted with immediately.
	 * @param name The name of the task (for user display)
	...
	/**
	 * Sets the TaskFactory used to create new Tasks.  Should be called by whatever controller is managing interaction
	 * with the users(s).
	 * @param f the Factory to use from here on out.
	 */
