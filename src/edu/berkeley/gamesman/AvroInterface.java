...
public class AvroInterface extends GamesmanApplication {
	...
	public void reallyRun(final int port) {
		...
		try {
			this.apiServer = new URI(serverConf.getProperty("avro.api.uri", "http://solvedgames.appspot.com/api/"));
		} catch (URISyntaxException e) {
			throw new RuntimeException(e);
		}

		GamestateProvider handler = new GamestateProvider();
		SpecificResponder processor = new SpecificResponder(GamesmanProvider.class, handler);
		HttpServer server;
		try {
		...
		try {
			...
			URI serverAddress = new URI("http", null,
					InetAddress.getLocalHost().getHostAddress(),
					port, "/", null, null);
			os.writeBytes("uri=" + URLEncoder.encode(serverAddress.toASCIIString(), "ASCII"));
			os.flush();
			os.close();
	...
	Configuration newLoadDatabase(Map<CharSequence, CharSequence> params, CharSequence game) {
		/*
	...
	synchronized Configuration addDatabase(Map<CharSequence, CharSequence> params,
			CharSequence game, String filename) {
	...
	private class GamestateProvider implements GamesmanProvider {
		...
		private <T extends State> PositionValue fillResponseFields(
				Configuration conf, T state, Fields fields) {
			// FIXME: Use fields....
		...
		public PositionValue getInitialPositionValue(CharSequence gameName,
				...
				throws AvroRemoteException {
			Configuration config = newLoadDatabase(variant, gameName);
			...
			if (chosenState != null) {
				return fillResponseFields(config, chosenState, fields);
			}
			return null;
		}
		...
		public Map<CharSequence, PositionValue> getPositionValues(
			...
			try {
				...
				for (CharSequence board : positions) {
					State state = game.synchronizedStringToState(board.toString());
					response.put(board, fillResponseFields(config, state, fields));
				}
		...
		public Map<CharSequence, PositionValue> getNextPositionValues(
			...
			System.out.println("getMoveValue request for \"" + gameName + "\": "
					+ variant);
			try {
				final Configuration config = newLoadDatabase(variant, gameName);
				final Fields syncFields = fields;
				final Game<State> game = config.getCheckedGame();
				final Map<CharSequence, PositionValue> syncMap = Collections.synchronizedMap(response);

				State state = game.synchronizedStringToState(position.toString());
				...
				for (Pair<String, State> moveTemp : game.synchronizedValidMoves(state)) {
					final Pair<String, State> move = moveTemp;
					threads.add(new Thread() {
						public void run() {
							syncMap.put(move.car, fillResponseFields(config, move.cdr, syncFields));
						}
					});
