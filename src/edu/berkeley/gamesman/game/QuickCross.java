 ...
 */
public final class QuickCross extends Game<QuickCrossState> implements
		Undoable<QuickCrossState> {
	...
	public Collection<QuickCrossState> startingPositions() {
		ArrayList<QuickCrossState> returnList = new ArrayList<QuickCrossState>(
				1);
	...
	public Collection<Pair<String, QuickCrossState>> validMoves(
		...
		QuickCrossState[] children = new QuickCrossState[pos.numPieces + 2
	...
	public int validMoves(QuickCrossState pos, QuickCrossState[] children) {
		...
		for (int i = 0; i < (boardSize); i++) {
			if (pos.getPiece(i) == ' ') {
				...
				throw new Error("cannot generate valid moves from given pos");
		}
		return numMoves;
	...
	public int possibleParents(QuickCrossState pos, QuickCrossState[] parents) {
		...
		for (int i = 0; i < boardSize; i++) {
			if (pos.getPiece(i) == '-') {
				...
				throw new Error("cannot generate a parent from given pos");
...
class QuickCrossState implements State<QuickCrossState> {
	final char[] board;
	private final int width;
	int numPieces = 0;
	...
	@Override
	public void set(QuickCrossState qcs) {
