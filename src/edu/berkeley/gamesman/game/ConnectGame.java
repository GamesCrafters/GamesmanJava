...
/**
 * A superclass for hex-style dartboard games in which the objective is to connect across the board somehow
 * 
 * @author dnspies
 */
public abstract class ConnectGame extends TierGame
{
    ...
    public ConnectGame( Configuration conf )
    {
        ...
        {
            /*
             * int boardSide = conf.getInteger("gamesman.game.outerRows", 2) + 2; boardSize = boardSide * (boardSide - 1) / 2 * 3;
             */
    ...
    public Collection< Pair< String, TierState >> validMoves()
    {
        ...
        ArrayList< Pair< String, TierState >> moves = new ArrayList< Pair< String, TierState >>( moveHashes.length );
        for ( int i = 0; i < moveHashes.length; i++ )
        {
            if ( moveHashes[i] >= 0 )
            {
                moves
                     .add( new Pair< String, TierState >( Integer.toString( translateOut( i ) ), newState( tier + 1, moveHashes[i] ) ) );
            }
        }
        return moves;
    }
