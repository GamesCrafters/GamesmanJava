...
SolveReader<NMMState, NMMRecord>	{
	...
	private int levelsOfBoxes; //represents how many boxes are in the board. in Typical NMM there are three concentric boxes
	 ...
	 * the third to last character is an int that has the number of X to be placed
	 * the second to last character is an int that has the number of O to be placed
	 * the last character is whose turn it is.
	 * 
	 */
	...
	public NMMState getPosition(String board) {
		...
		pos[gameSize+4] = Integer.valueOf(board.substring(gameSize+5, gameSize+5)).intValue();
	...
	/* We assume that the String board is in the following format:
	 * The first 24 characters in the string or either x or o, corresponding to pieces on the board
	 * the third to last character is an int that has the number of X pieces to be placed
	 * the second to last character is an int that has the number of O to be placed
	 * the last is a character specifying whose turn it is
	 */
	...
	public GameValue getValue(NMMState state) {
		return state.getValue(state.get(gameSize+5));
	}
		...
		for (int boxLevelofMoveTo = 0; boxLevelofMoveTo < levelsOfBoxes; boxLevelofMoveTo++) {
			for (int elementInBoxofMoveTo = 0; elementInBoxofMoveTo < elementsInABox; elementInBoxofMoveTo++) {
				...
				for (int numPieces = 3; numPieces < pieces; numPieces++) {
					Integer[] moveFromArray = generateMoveFroms(boxLevelofMoveTo, elementInBoxofMoveTo, numPieces);	
					for (int moveFrom : moveFromArray) {
						if(moveFrom != moveTo ) {
							//make the move without removing a piece from board
							allMoves.add(new Move(moveFrom,1,0, moveTo,0,1, gameSize+1,0,0, gameSize+5,1,2));
							allMoves.add(new Move(moveFrom,2,0, moveTo,0,2, gameSize,0,0, gameSize+5,2,1));
							...
							for (int removeFrom=0; removeFrom<gameSize; removeFrom++) {
								for (int onBoard = 0; onBoard < 9; onBoard++) {	
									...
									int diag1, diag2; //this is used to detect diagonal three in a rows
									if (elementInBoxofMoveTo % 2 == 0)  {//if it is on the corner.
										...
										allMoves.add(new Move(moveFrom,1,0, moveTo,0,1, mid1,1,1, corner1,1,1,
												removeFrom,2,0, gameSize+2,onBoard,onBoard-1, gameSize+1,0,0, gameSize+5,1,2));
										...
										allMoves.add(new Move(moveFrom,2,0, moveTo,0,2, mid1,2,2, corner1,2,2,
												removeFrom,1,0, gameSize+3,onBoard,onBoard-1, gameSize,0,0, gameSize+5,2,1));
									...
									// here we look at the "diagonals" the three in a rows that occur across different box levels
	...
	private Integer[] generateMoveFroms(int boxLevelofMoveTo, int elementInBoxofMoveTo, int numPieces) {
		int adj1;
		int adj2;
		...
		if (numPieces == 3) {
			...
			adj1 = ((elementInBoxofMoveTo + 1) % elementsInABox ) + boxLevelofMoveTo * levelsOfBoxes;
			adj2 = ((elementInBoxofMoveTo - 1) % elementsInABox ) + boxLevelofMoveTo * levelsOfBoxes;
			toReturn.add(adj1);
			toReturn.add(adj2);
		...
		return (Integer[]) toReturn.toArray();
	}
	...
	//TODO do this
	/*public boolean validMove(NMMState state, Move move) {
		return false;
	}
	 */
