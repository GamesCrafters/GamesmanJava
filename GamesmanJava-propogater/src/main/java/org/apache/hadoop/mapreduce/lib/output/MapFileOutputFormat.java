...
public class MapFileOutputFormat extends
	...
	public static List<MapFile.Reader> getReaders(Path[] dir, Configuration conf)
			throws IOException {
		FileSystem fs = dir[0].getFileSystem(conf);
		Path[] names = FileUtil.stat2Paths(fs.listStatus(dir));

		// sort names, so that hash partitioning works
		Arrays.sort(names);

		ArrayList<MapFile.Reader> parts = new ArrayList<MapFile.Reader>(
				names.length);
		for (int i = 0; i < names.length; i++) {
			if (names[i].getName().startsWith("_"))
				continue;
			parts.add(new MapFile.Reader(fs, names[i].toString(), conf));
		}
	...
	public static MapFile.Reader[] getReadersArray(Path[] paths,
			Configuration conf) throws IOException {
		List<MapFile.Reader> readers = getReaders(paths, conf);
		return readers.toArray(new MapFile.Reader[readers.size()]);
	}
}
