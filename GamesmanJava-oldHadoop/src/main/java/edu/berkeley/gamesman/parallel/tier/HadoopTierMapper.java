...
public class HadoopTierMapper extends
	...
	public void map(Range key, IntWritable value, final Context context)
		...
		try {
			...
			// Checking if p is already on hdfs => key is already solved
			final FileSystem fs = HDFSInfo.getHDFS(uri);
			if (fs.exists(p)) {
			...
			// Makes files delete-able for everyone in case of crash
