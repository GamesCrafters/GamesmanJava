...
/**
 * @author Eric
 * A class that keeps an array of range files that are used for database look up in the primitive pass and solver mappers
 * @param <S> The GameState class for the game being solved
 */
public class RangeFileManager<S extends State> {
	private RangeFile[] rangeFiles;
	private long hashesPerFile;

	/**
	 * @param fs the hadoop file system used in the mapper
	 * @param hadoopConf the hadoop configuration used in the mapper
	 * @param game the game being solved
	 */
	public RangeFileManager(FileSystem fs,
			org.apache.hadoop.conf.Configuration hadoopConf, Game<S> game) {
		try {
			SequenceFile.Reader reader;
			reader = new SequenceFile.Reader(fs, new Path(hadoopConf
					.get("db.map.path")), hadoopConf);
	...
	/**
	 * @param positionHash the hash of the position we're doing lookup for
