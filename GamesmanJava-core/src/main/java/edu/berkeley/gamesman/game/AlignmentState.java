...
public final class AlignmentState implements State<AlignmentState> {
	...
	private boolean[] guns2 = new boolean[] { false, false, false, false }; // guns
																			// exist
																			// far
																			// away,
																			// guns2[n][s][e][w]
	...
	/*
	 * for each position at (row, col), find guns pointing to that position from
	 * given direction We call this method four times in each direction under
	 * possibleParents. NOTE: NEED TO DOUBLE CHECK IF THE CORRECT COORDINATE
	 * SYSTEM WAS USED
	 */
	public int[] getGun(int row, int col, char direction, char opponent) {
		...
		case 'n':
			for (int i = row; i < board.length; i++) {
				// if the piece at this position matches opponent's piece, then
				// check if gun formed
				if (board[i][col] == opponent) {
					if ((i + 1 < board.length) && (col - 1 >= 0)
							&& (col + 1 < board.length)
							&& (board[i + 1][col - 1] == opponent)
							&& (board[i + 1][col + 1] == opponent)) {
						guns2[0] = true;
						coord[0] = col;
						coord[1] = i;
