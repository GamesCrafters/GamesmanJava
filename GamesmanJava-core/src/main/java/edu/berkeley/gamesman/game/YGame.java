...
public final class YGame extends ConnectGame {
	...
	private final int numberOfTriangles;// Total number of triangles (or rows,
										// rings, etc.)
	...
	public YGame(final Configuration conf) {
		...
		// Allocate and initialize the board, which is an array of character
		// arrays representing the triangles.
		...
		this.totalNumberOfNodes = (this.innerTriangleSegments + 1)
				* (this.innerTriangleSegments + 2) / 2
				+ (this.innerTriangleSegments * 2 + this.outerRows + 1)
				* this.outerRows / 2 * 3;
		...
		// ..and the neighbor vector ultimately returned from
		// getNeighbors/clockwiser

		this.neighbors = new Vector<Node>(6);

		this.fillBoardWithPlayer(' ');

		// Allocate and initialize a 2-dimensional array to use for plotting
		// ASCII the game board nodes.
	...
	public Vector<Node> getNeighbors(final int triangleIn, final int indexIn) {
		...
		if (segments == 0) /* Special case for point in the center */
		{
			...
			triangle = triangleIn;
			index = Util.nonNegativeModulo((indexIn + 1),
			...
			/* Inner neighbors: */
			if (this.isInnerTriangle(triangleIn) == false)/*
														 * Outer triangle to
														 * (outer triangle or
														 * transition triangle)
														 */
			{ /* 3 */
				...
				index = Util.nonNegativeModulo(indexIn - (indexIn / segments),
						this.nodesInThisTriangle[triangleIn - 1]);
				this.nodesOnInnerTriangle.add(board[triangle][index]);
				if (this.isCornerIndex(triangleIn, indexIn) == false)/*
																	 * The next
																	 * inner
																	 * neighbor
																	 * only when
				...
				/* if (isInnerTriangle(triangleIn) == false) */{
					if ((segments < 2)
							|| (this.isCornerIndex(triangleIn, indexIn) == true)) { /*
																					 * There
																					 * aren
																					 * 't
																					 * any
						...
						if (this.isCornerIndex(triangleIn, indexIn - 1) == true)/*
																				 * After
																				 * corner
																				 * index
																				 *//* 6 */
						{ /* 14 */
							triangle = triangleIn;
							index = Util.nonNegativeModulo(indexIn - 2,
									this.nodesInThisTriangle[triangleIn]);
							this.nodesOnSameTriangle
									.add(board[triangle][index]);
							if (segments > 3) {
								triangle = triangleIn - 1;
								/* 15 */index = Util
										.nonNegativeModulo(
						...
						if (segments == 2) { /* There are no inner neighbors */
						} else {
							if (segments == 3) {
								triangle = triangleIn - 1;
								index = 0;
								...
								if ((this
										.isCornerIndex(triangleIn, indexIn + 1) == false) /*
																						 * Not
																						 * a
																						 * corner
																						 * or
																						 ...
																						 * .
																						 */
										&& (this.isCornerIndex(triangleIn,
												indexIn - 1) == false)) { /* 21 */
									triangle = triangleIn - 1;
									// if (this.nodesInThisTriangle[triangleIn -
									// 1] > 0)
									// {
									index = Util
											.nonNegativeModulo(
			...
			/* Outer neighbors: */
			if (this.isOutermostRow(triangleIn) == false)/*
														 * Outer nodes have no
														 * neighbors.
														 */
			{
				if (this.isInnerTriangle(triangleIn + 1) == false)/*
																 * (Outer or
																 * transition
																 * triangle) to
																 * an outer
				...
				{ /* 9 */
					triangle = triangleIn + 1;
					index = Util.nonNegativeModulo(indexIn
							+ (indexIn / segments),
							this.nodesInThisTriangle[triangleIn + 1]);
	...
	public Vector<Node> clockwiser(final int triangleIn, final int indexIn) {
		assert ((this.nodesOnInnerTriangle.size() >= 0) && (this.nodesOnInnerTriangle
				.size() <= 3));

		boolean isAfterCorner = this.isCornerIndex(triangleIn, Util
				.nonNegativeModulo(indexIn - 1,
						this.nodesInThisTriangle[triangleIn]));

		boolean isBeforeCorner = this.isCornerIndex(triangleIn, Util
				.nonNegativeModulo(indexIn + 1,
						this.nodesInThisTriangle[triangleIn]));
		...
		// Handle a 2nd (of 3 or 4) "same triangle" neighbor (that are before
		// corners .. after corners have 1 on the same and 2
		// after the inner(s))
		...
		// Inners can be done by hand too, as there are only 0, 1, or 2 possible
		// neighbors. Inners go in descending order.

		if (this.nodesOnInnerTriangle.size() > 0) {
			if (this.nodesOnInnerTriangle.size() == 2) {
				// Handle index wrap-around for before corner nodes
				if (Util.nonNegativeModulo((indexIn + 1),
						this.nodesInThisTriangle[this.nodesOnInnerTriangle
								.get(0).triangle + 1]) == 0) {
					...
					// // The 2x4 triangle having 2 "inner nodes" that are both
					// on the same triangle.
					// // The 3x4 triangle having 1 "inner node" on the same
					// triangle and the other node at t:0,i:0.
					// // `-> which is different before and after the corners.
					// // The default case where inner indexes decrease for
					// clockwise order.
					// // The reason for the temporary variables is because the
					// logic of a combined if statement is uuuuuuuuuugly.
					//
					// // ALWAYS FALSE
					// // boolean hasOnlyOneSameTriangleInnerNode =
					// ((this.nodesOnInnerTriangle.get(0).triangle ==
					// triangleIn) &&
					// // (this.nodesOnInnerTriangle.get(1).triangle !=
					// triangleIn))||
					// // ((this.nodesOnInnerTriangle.get(0).triangle !=
					// triangleIn) && (this.nodesOnInnerTriangle.get(1).triangle
					// ==
					// // triangleIn));
					//
					// // ALWAYS FALSE
					// // boolean hasTwoSameTriangleInnerNodes =
					// ((this.nodesOnInnerTriangle.get(0).triangle ==
					// triangleIn) &&
					// // (this.nodesOnInnerTriangle
					...
					//
					// // ALWAYS -1
					// // int whichNodeIsClockwiseOnSameTriangleInnerNode =
					// hasTwoSameTriangleInnerNodes ? (this.nodesOnInnerTriangle
					// // int whichNodeIsClockwiseOnSameTriangleInnerNode =
					// false ? (this.nodesOnInnerTriangle.get(0).index == Util
					// // .nonNegativeModulo(indexIn + 2,
					// this.nodesInThisTriangle[triangleIn])) ? 0 : 1 : -1;
					//
					// int zeroZeroNodeIndex =
					// ((this.nodesOnInnerTriangle.get(0).triangle == 0) &&
					// (this.nodesOnInnerTriangle.get(0).index==0)) ? 0 :
					// ((this.nodesOnInnerTriangle.get(1).triangle == 0) &&
					// (this.nodesOnInnerTriangle.get(1).index==0)) ? 1 : -1;
					//
					// System.out.println(zeroZeroNodeIndex);
					//
					// // if ((((hasTwoSameTriangleInnerNodes == true) &&
					// (whichNodeIsClockwiseOnSameTriangleInnerNode == 0))
					// // if ((((false == true) &&
					// (whichNodeIsClockwiseOnSameTriangleInnerNode == 0))
					// if ((((false == true) && (-1 == 0))
					// || ((isAfterCorner == true) && (zeroZeroNodeIndex == 0))
					// // || ((isBeforeCorner == true) && (zeroZeroNodeIndex ==
					// 1)) || ((hasOnlyOneSameTriangleInnerNode ==
					// // false)
					// || ((isBeforeCorner == true) && (zeroZeroNodeIndex == 1))
					// || ((false == false)
					// // && (hasTwoSameTriangleInnerNodes == false) &&
					// (this.nodesOnInnerTriangle.get(0).index >
					// // this.nodesOnInnerTriangle
					// && (false == false) &&
					// (this.nodesOnInnerTriangle.get(0).index >
					// this.nodesOnInnerTriangle
					...
					// else
					// {
					int differenceInInnerIndex = Math
							.abs(this.nodesOnInnerTriangle.get(0).index
									- this.nodesOnInnerTriangle.get(1).index);

					if (((differenceInInnerIndex == 1) && (this.nodesOnInnerTriangle
							.get(0).index > this.nodesOnInnerTriangle.get(1).index))
							|| ((differenceInInnerIndex == 2) && (this.nodesOnInnerTriangle
									.get(0).index == 0))) {
						this.neighbors.add(this.nodesOnInnerTriangle.get(0));
						this.nodesOnInnerTriangle.remove(0);
					} else {
		...
		// Handle a 3rd (of 3 or 4) "same triangle" neighbor (that are before
		// corners .. after corners have 1 on the same and 2
		// after the inner(s))
		...
		// Outer neighbors are trickier, since there can be 0, 2, 3, 4, or 6
		// possible neighbors. 6 is handled specially above,
		// otherwise, sort by ascending order (which will handle the 4 with a
		// skip in the middle case). In corners, we add 2 to the
		// index to handle the wrap-around 0 cases.. only for corners, though,
		// since 2x4's will wrap the highest outer index to 0.

		if (this.nodesOnOuterTriangle.size() > 0) {
			int numberOfNodesOnOuterTriangle = this.nodesInThisTriangle[this.nodesOnOuterTriangle
					.get(0).triangle];
			...
			do {
				...
				for (int i = 1; i < this.nodesOnOuterTriangle.size(); i++) {
					...
					if (indexIn == 0) {
						index1ToCheck = Util
								.nonNegativeModulo(index1ToCheck + 2,
										numberOfNodesOnOuterTriangle);
						index2ToCheck = Util
								.nonNegativeModulo(index2ToCheck + 2,
										numberOfNodesOnOuterTriangle);
					}

					if (index1ToCheck > index2ToCheck) {
						Node tempNode = this.nodesOnOuterTriangle.get(i - 1);
						this.nodesOnOuterTriangle.set(i - 1,
								this.nodesOnOuterTriangle.get(i));
	 ...
	 * @param player
	 */
	public void setPlayerAt(final int triangle, final int index,
			final char player) {
	...
	// http://www.hexwiki.org/index.php?title=Programming_the_bent_Y_board, see
	// attached PNG for expected node numbers that we have to transform. This
	// should also be generalizable for smaller boards.

	private final double[][] coordsFor4and8board = { { 0.4958, 1.0000 },
			{ 0.6053, 0.9478 }, { 0.7153, 0.8768 }, { 0.8160, 0.7851 },
			{ 0.8988, 0.6748 }, { 0.9571, 0.5508 }, { 0.9901, 0.4209 },
			{ 1.0000, 0.2935 }, { 0.9935, 0.1757 }, { 0.8921, 0.1100 },
			{ 0.7739, 0.0532 }, { 0.6417, 0.0145 }, { 0.5018, 0 },
			{ 0.3617, 0.0129 }, { 0.2291, 0.0502 }, { 0.1101, 0.1056 },
			{ 0.0080, 0.1702 }, { 0, 0.2879 }, { 0.0084, 0.4155 },
			{ 0.0397, 0.5457 }, { 0.0966, 0.6704 }, { 0.1781, 0.7816 },
			{ 0.2777, 0.8744 }, { 0.3868, 0.9466 }, { 0.4963, 0.9049 },
			{ 0.6060, 0.8516 }, { 0.7079, 0.7786 }, { 0.7940, 0.6870 },
			{ 0.8581, 0.5806 }, { 0.8981, 0.4630 }, { 0.9137, 0.3414 },
			{ 0.9080, 0.2227 }, { 0.8056, 0.1574 }, { 0.6896, 0.1084 },
			{ 0.5649, 0.0820 }, { 0.4378, 0.0813 }, { 0.3127, 0.1064 },
			{ 0.1961, 0.1541 }, { 0.0929, 0.2182 }, { 0.0857, 0.3368 },
			{ 0.0998, 0.4586 }, { 0.1384, 0.5766 }, { 0.2012, 0.6838 },
			{ 0.2862, 0.7763 }, { 0.3872, 0.8504 }, { 0.4968, 0.8156 },
			{ 0.6011, 0.7608 }, { 0.6904, 0.6851 }, { 0.7569, 0.5948 },
			{ 0.8053, 0.4944 }, { 0.8295, 0.3819 }, { 0.8278, 0.2670 },
			{ 0.7267, 0.2070 }, { 0.6146, 0.1700 }, { 0.5008, 0.1592 },
			{ 0.3869, 0.1687 }, { 0.2744, 0.2045 }, { 0.1725, 0.2634 },
			{ 0.1694, 0.3782 }, { 0.1922, 0.4910 }, { 0.2394, 0.5920 },
			{ 0.3048, 0.6830 }, { 0.3931, 0.7596 }, { 0.4973, 0.7314 },
			{ 0.5908, 0.6719 }, { 0.6545, 0.5938 }, { 0.7056, 0.5090 },
			{ 0.7445, 0.4169 }, { 0.7522, 0.3088 }, { 0.6524, 0.2603 },
			{ 0.5509, 0.2458 }, { 0.4497, 0.2453 }, { 0.3480, 0.2586 },
			{ 0.2477, 0.3061 }, { 0.2540, 0.4142 }, { 0.2917, 0.5067 },
			{ 0.3418, 0.5921 }, { 0.4045, 0.6709 }, { 0.4978, 0.6503 },
			{ 0.5525, 0.5820 }, { 0.6026, 0.5076 }, { 0.6448, 0.4289 },
			{ 0.6795, 0.3491 }, { 0.5911, 0.3374 }, { 0.4998, 0.3324 },
			{ 0.4084, 0.3364 }, { 0.3199, 0.3471 }, { 0.3535, 0.4273 },
			{ 0.3947, 0.5065 }, { 0.4439, 0.5814 }, { 0.4987, 0.5035 },
			{ 0.5479, 0.4218 }, { 0.4505, 0.4213 } };
