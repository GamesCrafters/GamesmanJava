...
public final class Connect4 extends TierGame {
	...
	public final int gameHeight, gameWidth, gameSize;
	...
	@Override
	public int validMoves(TierState[] moves, int[] places) {
		int lenChildren = iah.getChildren(pieces.size() % 2 == 1 ? 'O' : 'X',
				children);
		int nextNumPieces = pieces.size() + 1;
		int col = 0;
	...
	public long recordToLong(TierState recordState, Record fromRecord) {
		if (conf.hasRemoteness) {
			...
			if (fromRecord.value == Value.UNDECIDED)
				...
				throw new Error(fromRecord.value + " not supported in Connect4");
		}
	}

	@Override
	public C4Cache getCache(Database db, long availableMem) {
		return new C4Cache(this, db, availableMem);
