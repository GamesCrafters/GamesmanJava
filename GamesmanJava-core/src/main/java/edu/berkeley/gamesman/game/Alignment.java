...
public class Alignment extends Game<AlignmentState> {
	protected final int gameWidth, gameHeight, gameSize; // changed to protected
															// for
															// AlignmentLoopy
	...
	private final AlignmentHasher myHasher; // changed to protected for
											// AlignmentLoopy
					...
					if (pl == pos.get(row, col)) {
						for (int adjRow = row - 1; adjRow <= row + 1; adjRow++) {
							for (int adjCol = col - 1; adjCol <= col + 1; adjCol++) {
								childPos.set(pos);
								if (childPos
										.legalMove(row, col, adjRow, adjCol)) {
									childStrings.add(Character
											.toString((char) ('a' + col))
											+ (row + 1)
											+ (char) ('a' + adjCol)
											+ (adjRow + 1));
					...
					if (pl == pos.get(row, col)) {
						for (int adjRow = row - 1; adjRow <= row + 1; adjRow++) {
							for (int adjCol = col - 1; adjCol <= col + 1; adjCol++) {
								...
								children[moves].set(pos);
								if (children[moves].movePiece(row, col, adjRow,
										adjCol, children[moves])) {
									// children[moves].set(pos);
