...
public final class DatabaseExplorer {
	/**
	 * The main method for exploring a database(jump to random hash and display)
	 * inside the console.
	 * 
	 * @param args
	...
	public static <S extends State<S>> void main(String[] args)
		...
		Game<S> g = conf.<S> getCheckedGame();
	...
	private static <S extends State<S>> void playGame(Game<S> g, Database db)
		...
		while (true) {
			...
			g.longToRecord(position, readRecord, storeRecord);
			...
			System.out
					.println("Primitive value: " + g.primitiveValue(position));
			System.out.println("Strict Primitive value: "
					+ g.strictPrimitiveValue(position));
