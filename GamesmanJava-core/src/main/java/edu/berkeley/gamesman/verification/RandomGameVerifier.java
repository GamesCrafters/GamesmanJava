...
public class RandomGameVerifier extends GameVerifier {
	...
	public boolean hasNext() {
		...
		if (!hasNext && Connect4CmdLineParser.debugging) {
			...
			System.out.println("Number of times first two tiers checked: "
					+ nFirstMovesChecked);
		}
		return hasNext;
	}
	 ...
	 * already been verified. This is just: P = 1 - P(not verified by run n) = 1
	 * - P(not verified on any run)^n = 1 - (1 - P(verified on any run))^n = 1 -
	 * (1 - (1 / width)^depth)^n
	 * 
