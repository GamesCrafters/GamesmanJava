	...
	public void childrenFunctions() {
		...
		do {
			...
			for (int i = 0; i < 9; i++) {
				...
				Assert.assertEquals(nextChildren[i], dh.nextChild(' ', 'O', i));
			...
			for (int i = numChildren - 1; i >= 0; i--) {
				for (lastPlace--; lastPlace > places[i]; lastPlace--) {
					...
					Assert.assertEquals(-1L, dh.getChild(' ', 'O', lastPlace));
				}
			...
			for (int i = 0; i < 9; i++) {
				...
				Assert.assertEquals(lastChildren[i],
						dh.previousChild(' ', 'O', i));
			}
		} while (dh.next());
		for (int i = 0; i < 9; i++) {
